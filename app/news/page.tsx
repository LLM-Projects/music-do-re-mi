          ...
          <div key={index} className="mb-16">
            <div className="grid gap-8 md:grid-cols-2 mb-8">
              ...
              <div>
                <div className="flex items-center gap-2 text-rose-500 dark:text-rose-400 mb-2">
                  ...
                  <time dateTime={item.date} className="font-semibold">{item.date}</time>
                </div>
            ...
            {item.achievements && (
              <div className="space-y-12">
                {Object.entries(item.achievements).map(([key, category]: [string, any]) => (
                  <div key={key} className="bg-slate-50 dark:bg-slate-900 rounded-lg p-8">
                    <div className="flex items-center gap-3 mb-6">
                      {key === 'piano' && <Music className="h-6 w-6 text-rose-500" />}
                      {key === 'clarinet' && <Music className="h-6 w-6 text-rose-500" />}
                      {key === 'other' && <Award className="h-6 w-6 text-rose-500" />}
                      <h4 className="text-xl font-bold">{category.title}</h4>
                    </div>
                    <div className="space-y-8">
                      {category.teachers.map((teacher: any, teacherIndex: number) => (
                        <div key={teacherIndex}>
                          <div className="flex items-center gap-2 mb-4">
                            <Users className="h-5 w-5 text-rose-500" />
                            <h5 className="text-lg font-semibold">Teacher: {teacher.name}</h5>
                          </div>
                          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                            {teacher.students.map((student: any, studentIndex: number) => (
                              <Card key={studentIndex} className="bg-white dark:bg-slate-800">
                                <CardHeader>
                                  <CardTitle className="text-base">{student.name}</CardTitle>
                                </CardHeader>
                                <CardContent>
                                  <div className="space-y-1">
                                    {student.awards.map((award: string, awardIndex: number) => (
                                      <div
                                        key={awardIndex}
